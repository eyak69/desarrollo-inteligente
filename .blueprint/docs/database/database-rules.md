...
- [BLOCKER] **Diccionario de Datos Autogenerado:** Queda estrictamente prohibido editar manualmente el archivo `database-dictionary.md`. Este documento debe ser el reflejo exacto y fiel del esquema físico de la base de datos. Tras crear y ejecutar cualquier migración de Knex, es obligatorio ejecutar el script de autogeneración (`node .blueprint/templates/scripts/generate-db-dictionary.js`) para mantener el diccionario de datos sincronizado antes de enviar un Pull Request.
- [BLOCKER] **Tipado TypeScript Dinámico y Automatizado:** Para evitar la deuda técnica de interfaces duplicadas y tipos obsoletos, se prohíbe declarar interfaces de TypeScript manuales para representar tablas de la base de datos. Se exige el uso de generadores automatizados (como `kanel` con `@kanel/knex` a través de `kanel.config.js`). Al modificar el esquema de la base de datos, el flujo de desarrollo debe incluir la regeneración automática de los tipos del backend (`npx kanel`).
